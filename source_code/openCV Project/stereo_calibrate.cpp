...
static void StereoCalib(const char* imageList, int nx, int ny, int useUncalibrated, float _squareSize)
{
    ...
    if( showUndistorted )
    {
		...
		#if DISPLAY_DISARITY_AND_RECTIFED==1
		cvNamedWindow( "rectified", 1 );


		// --- RECTIFY THE IMAGES AND FIND DISPARITY MAPS ----------------------------------:
        if( !isVerticalStereo )pair = cvCreateMat( imageSize.height, imageSize.width*2,CV_8UC3 );
        else pair = cvCreateMat( imageSize.height*2, imageSize.width, CV_8UC3 );
